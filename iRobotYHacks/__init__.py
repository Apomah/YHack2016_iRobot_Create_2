#This is the object file for Robot()
# Updated 7:02 am 13/11/16 
...
class Robot(object):
    ...
    def __init__(self, port='/dev/tty.usbserial-DA01NOM0', baud=115200):
        '''
    ...
    def setTurnSpeed(self, direction):
        if direction > 0:
            self.robot.turn_clockwise(direction)
        elif direction < 0:
            self.robot.turn_counter_clockwise(direction)

    def setTurn(self,speed,direction):
        if direction > 0:
            self.robot.turn_right(speed,direction)
        elif direction < 0:
            self.robot.turn_left(speed,direction)
    ...
    def getSelectedSensorData(self):
        self._get_sensor_packet()

        distance = robot.sensor_state ['distance']
        distance = distance if distance < 32768 else ~c^0xffff
        angle = robot.sensor_state['angle']
        angle = angle if angle < 32768 else ~c^0xffff
        encoderLeft = robot.sensor_state['left encoder counts']
        encoderLeft = encoderLeft if encoderLeft  < 32768 else ~c^0xffff
        encoderRight = robot.sensor_state['right encoder counts']
        encoderRight = encoderRight if encoderRight < 32768 else ~c^0xffff

        return distance,\
               angle,\
               self.robot.sensor_state['cliff left'],\
               self.robot.sensor_state['cliff front left'],\
               self.robot.sensor_state['cliff front right'],\
               self.robot.sensor_state['cliff right'],\
               self.robot.sensor_state['wall signal'],\
               self.robot.sensor_state['requested left velocity'],\
               self.robot.sensor_state['requested right velocity'],\
               encoderLeft,\
               encoderRight,\
               self.robot.sensor_state['light bump right signal'],\
               self.robot.sensor_state['light bump front right signal'],\
               self.robot.sensor_state['light bump center right signal'],\
               self.robot.sensor_state['light bump center left signal'],\
               self.robot.sensor_state['light bump front left signal'],\
               self.robot.sensor_state['light bump left signal'],\
               self.robot.sensor_state['light bumper']
...
class _Config(object):
    ...
    def load(self):
        ...
        """
        self.data = json.loads(pkg_resources.resource_string('iRobotYHacks', self.fname).decode('utf8'))
...
class _Create2(object):
    ...
    def drive_direct(self, velocityR, velocityL):
        
        data = []
        vL = None
        vR = None

        if velocityL >= -500 and velocityL <= 500:
            vL = int(velocityL) & 0xffff
            #Convert 16bit velocity to Hex
        else:
            noError = False
            raise _ROIDataByteError("Invalid velocity input")
        
        if velocityR >= -500 and velocityR <= 500:
            vR = int(velocityR) & 0xffff
            #Convert 16bit velocity to Hex
        else:
            noError = False
            raise _ROIDataByteError("Invalid velocity input")
    ...
    def turn_right(self, velocity, angle):
        ...
        velocityRight = int(velocity/abs(angle))
        self.drive_direct (velocityRight, velocityLeft)

    def turn_left(self, velocity, angle):
        velocityLeft = int(velocity/abs(angle))
